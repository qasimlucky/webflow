...
class WebhookDataService {
  static async getAllWebhookData(query = {}) {
    try {
      ...
      throw new AppError("Failed to retrieve webhook data", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getWebhookDataByEventType(eventType) {
    try {
      ...
      throw new AppError("Failed to retrieve webhook data by event type", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }

  static async getWebhookDataByStatus(status) {
    try {
      const webhooks = await WebhookData.find({ status }).sort({ received_at: -1 });
      ...
      throw new AppError("Failed to retrieve webhook data by status", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getWebhookDataByDateRange(startDate, endDate) {
    try {
      ...
      throw new AppError("Failed to retrieve webhook data by date range", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getWebhookDataWithRelations(webhookId) {
    try {
      ...
      const transactionId = webhook.payload?.transaction_id || webhook.payload?.transaction_data?.id;
      ...
      const result = await WebhookData.aggregate([
        ...
        },
        {
          $lookup: {
            ...
            let: { espBuchungId: { $arrayElemAt: ["$processMetadata.espBuchungId", 0] } },
            pipeline: [
              {
                $match: {
      ...
      throw new AppError("Failed to retrieve webhook data with relationships", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getAllWebhooksWithRelations(query = {}) {
    try {
      ...
      const webhooksWithRelations = await WebhookData.aggregate(aggregationPipeline);
      ...
      throw new AppError("Failed to retrieve webhooks with relationships", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getWebhooksByEventTypeWithRelations(eventType) {
    try {
      ...
      throw new AppError("Failed to retrieve webhooks by event type with relationships", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  static async getWebhookDataByTransactionId(transactionId) {
    try {
      ...
      throw new AppError("Failed to retrieve webhook data by transaction ID", HttpStatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
}
